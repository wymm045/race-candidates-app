...
def replace_today_candidates(races):
    ...
    for r in races:
        ...
        cur.execute(
            ...
            DO UPDATE SET
                ...
                ai_reasons = CASE WHEN EXCLUDED.ai_reasons IS NOT NULL AND EXCLUDED.ai_reasons <> '[]' THEN EXCLUDED.ai_reasons ELSE races.ai_reasons END,
                exhibition = CASE WHEN EXCLUDED.exhibition IS NOT NULL AND EXCLUDED.exhibition <> '[]' THEN EXCLUDED.exhibition ELSE races.exhibition END,
                exhibition_rank = CASE WHEN COALESCE(EXCLUDED.exhibition_rank, '') <> '' THEN EXCLUDED.exhibition_rank ELSE races.exhibition_rank END,
                motor_rank = CASE WHEN COALESCE(EXCLUDED.motor_rank, '') <> '' THEN EXCLUDED.motor_rank ELSE races.motor_rank END,
                ai_detail = CASE WHEN COALESCE(EXCLUDED.ai_detail, '') <> '' THEN EXCLUDED.ai_detail ELSE races.ai_detail END,
                ai_selection = CASE WHEN COALESCE(EXCLUDED.ai_selection, '') <> '' THEN EXCLUDED.ai_selection ELSE races.ai_selection END,
    ...
    cur.close()
    conn.close()
    log(f"replace_today_candidates race_date={race_date} inserted={inserted} updated={updated} deleted={deleted}")
...
.multi-check-wrap input[type="checkbox"] {{ width:20px; height:20px; }}
.card-top {{ display:flex; justify-content:space-between; align-items:flex-start; gap:12px; margin-bottom:12px; }}
.card-top-left {{ display:flex; flex-direction:column; gap:10px; min-width:0; }}
