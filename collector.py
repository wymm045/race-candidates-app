...
def parse_single_race_deadline(jcd, race_no):
    ...
    for i, line in enumerate(lines):
        if "締切予定時刻" in line:
            ...
            candidates.extend(re.findall(r"\d{2}:\d{2}", block))
...
def parse_beforeinfo_for_key(jcd, race_no):
    ...
    try:
        ...
        log(f"[beforeinfo_error] jcd={jcd} venue={venue} race_no={race_no} err={e}")
        return (jcd, race_no), {
    ...
    if times:
        log(f"[beforeinfo_ok] jcd={jcd} venue={venue} race_no={race_no} exhibition=ok")
    else:
        log(f"[beforeinfo_partial] jcd={jcd} venue={venue} race_no={race_no} exhibition=empty")
