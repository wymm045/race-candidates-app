...
def analyze_candidate(official_rating, selection, exhibition_info, boat_stats=None, environment=None, class_history_map=None, extra_stats=None):
    ...
    for t in triplets:
        ...
        try:
            a, b, c = map(int, parts)
            heads.append(a); seconds.append(b); thirds.append(c)
        except Exception:
            continue
    ...
    if len(unique_heads) == 1 and heads:
        score += 0.9; reasons.append("1着候補がかなり絞れている")
    elif len(unique_heads) == 2 and heads:
        score += 0.5; reasons.append("1着候補が比較的絞れている")
    elif len(unique_heads) >= 4:
        score -= 0.6; reasons.append("1着候補が散っている")
    if len(unique_seconds) <= 3 and seconds:
        score += 0.3; reasons.append("2着候補が比較的絞れている")
    elif len(unique_seconds) >= 5:
        score -= 0.2; reasons.append("2着候補が広い")
    if len(unique_thirds) <= 4 and thirds:
        score += 0.2; reasons.append("3着候補が比較的整理されている")
    if unique_heads and min(unique_heads) >= 4:
        score -= 0.6; reasons.append("外枠1着中心")
    ...
    if exhibition_ranks:
        if 1 in exhibition_ranks:
            ...
            if r1 == 1:
                score += 0.8; reasons.append("1号艇の展示順位が1位")
            elif r1 <= 2:
                score += 0.4; reasons.append("1号艇の展示順位が上位")
            elif r1 >= 5:
                score -= 0.6; reasons.append("1号艇の展示順位が下位")
        ...
        if head_ranks:
            ...
            if head_avg_rank <= 1.8:
                score += 1.2; reasons.append("1着候補の展示順位がかなり良い")
            elif head_avg_rank <= 2.5:
                score += 0.7; reasons.append("1着候補の展示順位が良い")
            elif head_avg_rank <= 3.2:
                score += 0.3; reasons.append("1着候補の展示順位がまずまず")
            elif head_avg_rank >= 4.5:
                score -= 1.0; reasons.append("1着候補の展示順位が悪い")
        ...
        if head_times and all_times:
            ...
            if exhibition_spread >= 0.18:
                if head_time_gap_from_top <= 0.03:
                    score += 0.35; reasons.append("展示タイム差が大きく1着候補がかなり優勢")
                elif head_time_gap_from_top >= 0.12:
                    score -= 0.25; reasons.append("展示タイム差がある中で1着候補が遅い")
    ...
    if boat_stats:
        ...
        if head_national is not None:
            ...
            if head_national >= 6.2:
                score += 0.9; reasons.append("1着候補の全国勝率がかなり高い")
            elif head_national >= 5.5:
                score += 0.5; reasons.append("1着候補の全国勝率が高い")
            elif head_national < 4.8:
                score -= 0.5; reasons.append("1着候補の全国勝率が低い")
        if head_local is not None:
            details.append(f"1着当地勝率平均{round(head_local, 2)}")
            if head_local >= 6.0:
                score += 0.5; reasons.append("1着候補の当地勝率が高い")
            elif head_local < 4.8:
                score -= 0.3; reasons.append("1着候補の当地勝率が低い")
        if head_motor is not None:
            details.append(f"1着モーター2連率平均{round(head_motor, 1)}")
            if head_motor >= 42:
                score += 0.6; reasons.append("1着候補のモーター気配が良い")
            elif head_motor < 30:
                score -= 0.3; reasons.append("1着候補のモーターが弱い")
        if head_boat is not None:
            details.append(f"1着ボート2連率平均{round(head_boat, 1)}")
            if head_boat >= 38:
                score += 0.2; reasons.append("1着候補のボート気配が良い")
            elif head_boat < 30:
                score -= 0.2; reasons.append("1着候補のボート気配が弱い")
    ...
    if head_histories:
        ...
        if avg_head_class >= 1.6:
            score += 0.95; reasons.append("1着候補の級別3期傾向がかなり強い")
        elif avg_head_class >= 1.0:
            score += 0.55; reasons.append("1着候補の級別3期傾向が良い")
        elif avg_head_class <= -0.3:
            score -= 0.50; reasons.append("1着候補の級別3期傾向が弱い")
    if second_histories:
    ...
    recent_vals = [x.get("recent_avg") for x in head_extra if isinstance(x.get("recent_avg"), (int, float))]
    top3_vals = [x.get("recent_top3") for x in head_extra if isinstance(x.get("recent_top3"), (int, float))]
    if course_vals:
        avg_course = sum(course_vals) / len(course_vals)
        details.append(f"1着枠別3連対率平均{round(avg_course,1)}")
        if avg_course >= 70:
            score += 0.55; reasons.append("1着候補の枠別相性がかなり良い")
        elif avg_course >= 55:
            score += 0.28; reasons.append("1着候補の枠別相性が良い")
        elif avg_course <= 30:
            score -= 0.30; reasons.append("1着候補の枠別相性が弱い")
    if st_vals:
        avg_st = sum(st_vals) / len(st_vals)
        details.append(f"1着平均ST{round(avg_st,2)}")
        if avg_st <= 0.14:
            score += 0.18; reasons.append("1着候補の平均STが良い")
        elif avg_st >= 0.20:
            score -= 0.14; reasons.append("1着候補の平均STが遅め")
    if recent_vals:
        avg_recent = sum(recent_vals) / len(recent_vals)
        details.append(f"1着直近平均着順{round(avg_recent,2)}")
        if avg_recent <= 2.2:
            score += 0.25; reasons.append("1着候補の直近成績が良い")
        elif avg_recent >= 4.2:
            score -= 0.22; reasons.append("1着候補の直近成績が悪い")
    if top3_vals:
        avg_top3 = sum(top3_vals) / len(top3_vals)
        details.append(f"1着直近3着内率{round(avg_top3,1)}")
        if avg_top3 >= 80:
            score += 0.18
        elif avg_top3 <= 40:
            score -= 0.14
        ...
        if wind_speed >= 4:
            if wind_type == "headwind":
                if 1 in unique_heads:
                    score += 0.35; reasons.append("向かい風で1号艇寄り")
                if outer_heads:
                    score -= 0.12; reasons.append("向かい風で外頭は少し不利")
            elif wind_type == "tailwind":
                if outer_heads:
                    score += 0.25; reasons.append("追い風で外の一撃候補を少し評価")
            elif wind_type == "crosswind":
                if outer_heads:
                    score -= 0.12; reasons.append("横風で外頭は少し割引")
        if wind_speed >= 6:
            if 1 in unique_heads:
                score += 0.15; reasons.append("強風で内寄りを少し評価")
            if very_outer_heads:
                score -= 0.20; reasons.append("強風で大外頭は少し厳しい")
    if wave_height is not None:
        if wave_height >= 5:
            if 1 in unique_heads:
                score += 0.12
            if outer_heads:
                score -= 0.12; reasons.append("波高高めで外頭を少し割引")
        if wave_height >= 7:
            if 1 in unique_heads:
                score += 0.10
            if very_outer_heads:
                score -= 0.15; reasons.append("波高かなり高めで大外頭をさらに割引")
    if stabilizer:
        if 1 in unique_heads:
            score += 0.18; reasons.append("安定板ありでイン寄りを少し評価")
        if outer_heads:
            score -= 0.18; reasons.append("安定板ありで外のまくり頭を少し割引")
    ...
    return {"ai_score": round(score, 2), "ai_rating": ai_rating, "ai_label": "", "final_rank": final_rank, "ai_reasons": reasons, "exhibition": exhibition_times, "exhibition_rank": exhibition_rank_text, "motor_rank": "", "ai_detail": " / ".join(details) if details else ""}
...
def build_candidates():
    log("[collector_version] ai_recent_course_v3_coursefix")
